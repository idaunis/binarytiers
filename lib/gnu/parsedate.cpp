...
const char * const bt_wkday[] = {"Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"};
static const char * const weekday[] = { "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday" };
const char * const bt_month[]= { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
...
/*
 * bt_raw_equal() is for doing "raw" case insensitive strings. This is meant
...
int bt_raw_equal(const char *first, const char *second)
{
  while(*first && *second) {
    if(bt_raw_toupper(*first) != bt_raw_toupper(*second))
  ...
  return (bt_raw_toupper(*first) == bt_raw_toupper(*second));
}

int bt_raw_nequal(const char *first, const char *second, size_t max)
{
 ...
 */
void bt_strntoupper(char *dest, const char *src, size_t n)
{
  if (n < 1)
    return;

  do {
    *dest++ = bt_raw_toupper(*src);
  } while (*src++ && --n);
}
...
static int checktz(const char *check)
{
  ...
  for(i=0; i< sizeof(tz)/sizeof(tz[0]); i++) {
    if(bt_raw_equal(check, what->name)) {
      found= true;
      break;
    }
    what++;
