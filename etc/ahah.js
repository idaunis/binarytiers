 ...
 *
 * Wuaala uses this file to enhance form elements with #ahah[path] and
...
function ahah_init() {
  for (var key in ahah_settings) {
    if (!$('#'+ key + '.ahah-processed').size()) {
      var element_settings = ahah_settings[key];
      $(element_settings.selector).each(function() {
        element_settings.element = this;
        var ahah = new AHAH(key, element_settings);
      });
      $('#'+ key).addClass('ahah-processed');
    }
  }
}
 ...
 */
AHAH.prototype.beforeSubmit = function (form_values, element, options) {
  // Disable the element that received the change.
  $(this.element).addClass('progress-disabled').attr('disabled', true);

  // Insert progressbar or throbber.
  if (this.progress.type == 'bar') {
    var progressBar = new ProgressBar('ahah-progress-' + this.element.id, eval(this.progress.update_callback), this.progress.method, eval(this.progress.error_callback));
 ...
 */
AHAH.prototype.success = function (response, status) {
 ...
 */
AHAH.prototype.error = function (response, uri) {
